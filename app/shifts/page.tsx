...
export default function ShiftsPage() {
  ...
  const fetchShifts = async () => {
    try {
      ...
      const normalizedShifts = Array.isArray(data)
        ? data
        : Array.isArray((data as any)?.shifts)
          ? (data as any).shifts
          : [];

      const normalizedAssignments = Array.isArray(assignmentData)
        ? assignmentData
        : Array.isArray((assignmentData as any)?.assignments)
          ? (assignmentData as any).assignments
          : [];

      const normalizedEmployees = Array.isArray(employeeData)
        ? employeeData
        : Array.isArray((employeeData as any)?.employees)
  ...
  const pageSubtitle = useMemo(
    () => "Create shifts, assign them to employees, and track assignment status.",
    []
  );
